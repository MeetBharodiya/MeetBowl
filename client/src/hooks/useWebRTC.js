...
export const useWebRTC = (roomId, user) => {
  ...
  //TO put some wrappers before set any newClient 
  const addNewClient = useCallback((newClient, cb) => {
    //check whether the client is already connected or not
    const lookingFor = clients.find((client) => client.id === newClient.id);

    //if not then setnewClient with existingclients
    if (lookingFor === undefined) {
  ...
  // To capture the audio/Media of any instances that are connected to the room 
  useEffect(() => {
    ...
    // console.log("hello", localMediaStream.current);
    startCapture().then(() => {
      addNewClient({...user,muted:true}, () => {
        const localElement = audioElements.current[user.id];
        if (localElement) {
  ...
  // Store instances for manipulate the audio of any instances
  const provideRef = (instance, userId) => {
    audioElements.current[userId] = instance;
  }
